...
function resolveRuntimeConfig(messages?: MessagesConfig): RuntimeConfig {
  const configured = messages?.operationalAlerts;
  return {
    enabled: configured?.enabled ?? Boolean(configured),
    sources: new Set(
      configured?.sources ?? [
        "tool",
        "agent",
...
function resolveUserFallbackRoute(
  ...
  const entry = loadSessionStore(storePath)[normalizedSessionKey];
  const target = resolveSessionDeliveryTarget({ entry });
  if (!target.channel || !target.to) return undefined;
  const resolved = resolveOutboundTarget({
...
function pruneDedupeCache(now: number, windowMs: number): void {
  for (const [key, ts] of dedupeCache.entries()) {
    if (now - ts > windowMs) dedupeCache.delete(key);
  }
...
export async function routeOperationalAlert(params: {
  cfg: OpenClawConfig;
  ...
  try {
    ...
    if (fallbackRoute) {
      try {
        ...
        log.warn("operational alert fallback delivery failed", {
          error: error.message,
