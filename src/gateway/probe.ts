...
export async function probeGateway(opts: {
  ...
  return await new Promise<GatewayProbeResult>((resolve) => {
    ...
    const client = new GatewayClient({
      ...
      instanceId,
      // Keep device identity enabled for loopback probes too, so operator-scoped
      // status/config RPCs can use the stored paired operator scopes instead of
      // degrading into a connected-but-scope-less local token session.
